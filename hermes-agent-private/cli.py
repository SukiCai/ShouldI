...
class HermesCLI:
    ...
    def __init__(
        ...
        print(f"Loaded = = ={(self.prefill_messages)} = = = prefill messages from config.")
    ...
    def _handle_profile_command(self, args: str = ""):
        """Display active profile name, home directory, and user model if active."""
        ...
        # Show user model when user_model provider is active
        try:
            from plugins.memory.user_model.store import UserModelStore

            # Get user_id from active memory provider if available
            user_id = ""
            if (hasattr(self, "agent") and self.agent and
                    hasattr(self.agent, "_memory_manager")):
                provider = self.agent._memory_manager.get_provider("user_model")
                if provider:
                    user_id = getattr(provider, "_user_id", "")
            ...
            # Handle /profile clear <field>
            parts = args.strip().split()
            if parts and parts[0].lower() == "clear":
                if len(parts) < 2:
                    print("  Usage: /profile clear <field>")
                    print("  Example: /profile clear communication_style")
                    print()
                    return
                field = parts[1]
                removed = store.remove_inferred_dimension(user_id, field)
                if removed:
                    print(f"  Removed inferred dimension: {field}")
                else:
                    print(f"  Field '{field}' not found in user model.")
                print()
                return

            model = store.get_model(user_id)
            if not model:
                print(f"  No user model found for user_id: {user_id}")
                print()
                return

            print(f"  -- User Model (user_id: {user_id}) --")
            print()
            ...
            # Inferred dimensions
            if model.inferred:
                print("  Inferred Traits:")
                for dim in sorted(model.inferred, key=lambda d: sum(k.confidence for k in d.keywords) / max(len(d.keywords), 1), reverse=True):
                    avg = sum(k.confidence for k in dim.keywords) / len(dim.keywords)
                    print(f"    [{dim.field}]  avg confidence: {avg:.2f}")
                    if dim.summary:
                        print(f"      {dim.summary}")
                    for kw in sorted(dim.keywords, key=lambda k: k.confidence, reverse=True):
                        print(f"      · {kw.value}  ({kw.confidence:.2f})")
                    print()
            else:
                print("  No inferred traits yet.")
                print()

            print("  Tip: /profile clear <field>  — remove a specific inferred dimension")
            print()
        except Exception:
            pass  # user_model not active or user not found — silent
    ...
    def process_command(self, command: str) -> bool:
        ...
        if canonical in {"quit", "exit"}:
            ...
            _profile_args = cmd_lower[len("profile"):].strip() if "profile" in cmd_lower else ""
            self._handle_profile_command(_profile_args)
